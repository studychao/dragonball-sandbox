...
use vm_allocator::{Constraint, IntervalTree, NodeState, Range};
...
impl PciBus {
    ...
    pub fn get_device(&self, device_id: u32) -> Option<Arc<dyn PciDevice>> {
        ...
        match devices.get(&Range::new(device_id as u64, device_id as u64)) {
            Some(node) => match node {
                NodeState::Valued(d) => Some(d.clone()),
                _ => None,
            },
            None => None,
        }
    ...
    pub fn read_config(&self, dev: u32, func: u32, offset: u32, data: &mut [u8]) {
        if dev > 0x1f
            || func != 0
            || offset >= 0x1000
            || offset & (data.len() - 1) as u32 & 0x3 != 0
        {
    ...
    /// a read write lock, we can't return DeviceResources's address for caller useing.
            ...
            match req {
                ResourceConstraint::PioAddress { range, align, size } => {
                    ...
                    match state.ioport_resources.allocate(&constraint) {
                        ...
                        None => {
                            Self::i_free_resources(state, resources);
    ...
    fn i_free_resources(mut state: RwLockWriteGuard<PciBusContent>, resources: DeviceResources) {
...
mod tests {
    ...
    fn test_bus_allocate_resource() {
        ...
        assert!(bus
            .assign_resources(DeviceResources::from(resources))
            .is_ok());

...
use vm_device::resources::{DeviceResources, Resource, ResourceConstraint};

use crate::{Error, PciBus, Result};

#[cfg(feature = "vm-snapshot")]
#[path = "configuration_persist.rs"]
pub mod persist;
...
pub trait PciCapability: Send + Sync + Downcast {
...
downcast_rs::impl_downcast!(PciCapability);
...
impl Default for PciBarPrefetchable {
    fn default() -> Self {
...
pub struct PciConfiguration {
    ...
    capabilities: Vec<(usize, usize, Arc<Mutex<Box<dyn PciCapability>>>)>,
    bus: Weak<PciBus>,
}
...
impl PciConfiguration {
    ...
    pub fn write_u32(&mut self, offset: usize, value: u32) -> bool {
        ...
        // Handle registers for normal PCI devices.
        if self.header_type == PciHeaderType::Device && reg_idx < 16 {
            if (reg_idx >= BAR0_REG && reg_idx < BAR0_REG + NUM_BAR_REGS) || reg_idx == ROM_BAR_REG
            {
...
impl PciConfiguration {
    fn write_device_bar_reg(&mut self, reg_idx: usize, value: u32) -> bool {
        debug_assert!(
            (reg_idx >= BAR0_REG && reg_idx < BAR0_REG + NUM_BAR_REGS) || reg_idx == ROM_BAR_REG
        );
    ...
    fn allocate_bar_resource(&mut self, param: &BarProgrammingParams) {
        ...
        assert_eq!(self.bar_allocated(param.bar_idx), false);
...
pub(crate) mod tests {
    ...
    impl PciCapability for TestCap {
        ...
        fn interrupt_type(&self) -> PciCapabilityID {
    ...
    fn add_capability() {
        ...
        assert_eq!(config.write_u16(cap1_offset + 4, 0x1111), true);
        ...
        assert_eq!(config.write_u32(cap1_offset + 8, 0xa5a5_a5a5), true);
        let (handled, cap1_data) = config.read_u32(cap1_offset + 8);
        assert_eq!(handled, true);
    ...
    fn test_bar_configuration() {
        ...
        assert_eq!(config.prefetchable(), false);
    ...
    fn test_write_device_bar_reg() {
        ...
        // Detecting BAR size
        assert_eq!(config.write_u32(0x10, 0xffffffff), true);
        ...
        config.add_device_bar(&bar).unwrap();
        assert_eq!(config.write_u32(0x18, 0xffff_ffff), true);
        assert_eq!(config.get_bar_programming_params(), None);
        assert_eq!(config.write_u32(0x1c, 0xffff_ffff), true);
        assert_eq!(config.get_bar_programming_params(), None);
        assert_eq!(config.read_u32(0x18), (true, 0x4));
        assert_eq!(config.read_u32(0x1c), (true, 0xffff_fffe));

        assert_eq!(config.write_u32(0x18, 0x2000_0000), true);
        assert_eq!(config.get_bar_programming_params(), None);
        assert_eq!(config.write_u32(0x1c, 0x0000_0002), true);
        ...
        assert_eq!(config.bar_allocated(params.bar_idx), true);

        assert_eq!(config.write_u32(0x20, 0xffff_ffff), true);
    ...
    fn test_update_rom_bar() {
        ...
        assert_eq!(config.write_u32(0x30, 0xffffffff), true);
        ...
        assert_eq!(config.write_u32(0x30, 0xe0001000), true);
        assert_eq!(config.get_bar_programming_params(), None);

        // Write the BAR with a different address
        assert_eq!(config.write_u32(0x30, 0xc0001000), true);
        ...
        assert_eq!(config.write_u32(0x30, 0x00000000), true);
        ...
        assert_eq!(params.len, 0x1000_0000);
        assert_eq!(params.bar_type, PciBarRegionType::Memory32BitRegion);
        assert_eq!(config.bar_allocated(params.bar_idx), false);
    }
    ...
    fn test_read_write_u32() {
        ...
        assert_eq!(config.write_u32(1, 0x1), false);
        assert_eq!(config.write_u32(0x0, 0), true);
        assert_eq!(config.read_u32(0x0), (true, 0x0386_8086));
        assert_eq!(config.write_u32(0x4, 0xffff_ffff), true);
        assert_eq!(config.read_u32(0x4), (true, 0x0000_ffff));
        assert_eq!(config.write_u32(0x8, 0xffff_ffff), true);
        assert_eq!(config.read_u32(0x8), (true, 0x0201_5a00));
        assert_eq!(config.write_u32(0xc, 0xffff_ffff), true);
        assert_eq!(config.read_u32(0xc), (true, 0x0000_00ff));
        assert_eq!(config.write_u32(0x28, 0xffff_ffff), true);
        assert_eq!(config.read_u32(0x28), (true, 0x0000_0000));
        assert_eq!(config.write_u32(0x2c, 0xffff_ffff), true);
        assert_eq!(config.read_u32(0x2c), (true, 0x2468_abcd));
        assert_eq!(config.write_u32(0x34, 0xffff_ffff), true);
        assert_eq!(config.read_u32(0x34), (true, 0x0000_0000));
        assert_eq!(config.write_u32(0x38, 0xffff_ffff), true);
        assert_eq!(config.read_u32(0x38), (true, 0x0000_0000));
        assert_eq!(config.write_u32(0x3c, 0xffff_ffff), true);

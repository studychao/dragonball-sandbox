...
pub fn create_pci_root_bus(bus_id: u8) -> Result<Arc<PciBus>> {
    ...
    let dev = Arc::new(PciHostBridge::new(id as u8, Arc::downgrade(&bus))?);
...
mod tests {
    ...
    use vm_device::resources::{DeviceResources, Resource};
    use vm_device::PioAddress;

    #[test]
    fn test_create_pci_root_bus() {
        let root_bus = create_pci_root_bus(0).unwrap();
        let host_bridge = PciHostBridge::new(0, Arc::downgrade(&root_bus.clone()));

        assert_eq!(root_bus.bus_id(), 0);
        assert_eq!(host_bridge.id(), 0);
    ...
    fn test_read_pci_root_root_bus_cfg() {
        ...
        root.add_bus(root_bus.clone(), 0).unwrap();

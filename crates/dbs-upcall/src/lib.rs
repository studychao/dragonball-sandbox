// Copyright 2019 Alibaba Corporation. All Rights Reserved.
...
impl<S: UpcallClientService + Send + 'static> UpcallClient<S> {
    ...
    pub fn connect(&mut self) -> Result<()> {
        let handler = Box::new(UpcallEpollHandler::new(self.info.clone(), &self.logger)?);
...
impl<S: UpcallClientService + Send> UpcallEpollHandler<S> {
    fn new(info: Arc<Mutex<UpcallClientInfo<S>>>, logger: &slog::Logger) -> Result<Self> {
    ...
    fn set_reconnect(&mut self) -> Result<()> {
        if self.in_reconnect {
            slog::info!(self.logger, "upcall server is waiting for reconnect");
            return Ok(());
        }
        ...
        match info.state {
            ...
            UpcallClientState::WaitingService => {
                if let Err(e) = info.service_connection_check() {
                    slog::warn!(self.logger, "upcall connect service check failed, {}", e);
                    info.set_state(UpcallClientState::WaitingServer);
                    if let Err(e) = self.set_reconnect() {
                        slog::error!(self.logger, "set reconnect error: {}", e);
                        info.set_state(UpcallClientState::ReconnectError);
                    ...
                    slog::info!(self.logger, "upcall connect service success");
                    info.set_state(UpcallClientState::ServiceConnected);
                }
            }
            UpcallClientState::ServiceBusy => match info.handle_response() {
                Ok(response) => {
                    slog::trace!(self.logger, "upcall handle response success");
                    info.set_state(UpcallClientState::ServiceConnected);
                    info.consume_callback(response);
                }
                Err(e) => {
                    slog::warn!(self.logger, "upcall response failed {}", e);
                    info.set_state(UpcallClientState::WaitingServer);
                    if let Err(e) = self.set_reconnect() {
                        slog::error!(self.logger, "set reconnect error: {}", e);
                        info.set_state(UpcallClientState::ReconnectError);
            ...
            UpcallClientState::ServiceConnected | UpcallClientState::ReconnectError => {
                slog::error!(self.logger,"we should get message from event handler when connection state is `ServiceConnected`");
            }
        }
    ...
    fn handle_reconnect_event(&mut self, ops: &mut EventOps) {
        ...
        if let Err(e) = info.server_connection_start() {
            slog::warn!(self.logger, "upcall reconnect server /failed: {}", e);
            if let Err(e) = self.set_reconnect() {
                slog::error!(self.logger, "set reconnect error: {}", e);
            }
        }
        slog::info!(self.logger, "upcall reconnect server...");
    ...
    S: UpcallClientService + Send + 'static,
{
    fn process(&mut self, events: Events, ops: &mut EventOps) {
        slog::trace!(self.logger, "UpcallEpollHandler: process");
        ...
        match events.fd() {
            ...
            _ => slog::error!(self.logger, "upcall epoll handler: unknown event"),
        }
    }

    fn init(&mut self, ops: &mut EventOps) {
        slog::trace!(self.logger, "UpcallEpollHandler: init");
...
mod tests {
    ...
    impl UpcallClientService for FakeService {
        fn connection_start(&self, stream: &mut Box<dyn VsockStream>) -> Result<()> {
            ...
            stream
                .write(&String::from("CONN START").into_bytes())
                .unwrap();
            Ok(())
        }
        fn connection_check(&self, stream: &mut Box<dyn VsockStream>) -> Result<()> {
        ...
        fn send_request(
            ...
            stream
                .write(&String::from("TEST REQ").into_bytes())
                .unwrap();
            Ok(())
        }
    ...
    fn test_upcall_client_info_server_connection_start_and_check() {
        ...
        inner_stream.write(&writer_buffer).unwrap();
    ...
    fn test_upcall_client_send_request() {
        ...
        let writer_buffer = String::from("TEST RESP").into_bytes();
        assert!(inner_stream.write(&writer_buffer.as_slice()).is_ok());
        ...
        assert_eq!(*got_response.lock().unwrap(), true);
    }
    ...
    fn test_upcall_epoll_handler_set_reconnect() {
        ...
        assert_eq!(epoll_handler.in_reconnect, true);
